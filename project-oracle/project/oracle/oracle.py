...
class Oracle:
    ...
    def billboardjs_conversion(self, plot_type, x, y, is_timeseries):
        data = {}
        if is_timeseries:
          xs, ys = ['x'], ['data']
          data['x'] = 'x'
    ...
    def match_expr(self, entity, b_list, title, e_filters):
        ...
        if b_list is not None:  # if b_list is not empty, perform a plot over distribution of feature
            ...
            if plot_l and dist_plot:  # if calculated results are returned, generate the plot
                plot = self.billboardjs_conversion('bar', names, plot_l, False)
            ...
            if entity in self.config.module_keywords:
                ...
                if plot_l:
                    ...
                    names = [name.strftime('%Y-%m-%d') for name in names]
                    plot = self.billboardjs_conversion('line', names, plot_l, True)
                    # plot = pd.DataFrame({'x': names, 'y': plot_l})
    ...
    def sequential_filter(self, f_list):
        ...
        for f_key in filters:
            ...
            for i in range(len(f_key)):  # iterate over each filtering criterion
                f_, c_, conj_ = f_key[i][0], f_key[i][1], f_key[i][
    ...
    def automate_plot_by_(self, x, y, entity, title, chart_type):
        ...
        return py.iplot(fig, filename='extract_bar') if chart_type == 'bar' else py.iplot(fig,
                                                                                          filename='extract_scatter')
